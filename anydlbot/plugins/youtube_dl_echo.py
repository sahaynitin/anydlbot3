...
async def echo(bot, update):
    if Config.LOG_CHANNEL:
        try:
            log_message = await log_message.forward(Config.LOG_CHANNEL)
    ...
    if t_response:
        ...
        if "formats" in response_json:
            for formats in response_json["formats"]:
                ...
                "Video 🎥", callback_data=f"cb_string_video.encode("UTF-8")||{format_string}||{format_ext}"), InlineKeyboardButton("Document 🗂", callback_data=f"cb_string_file.encode("UTF-8")||{format_string}||{format_ext}")
